...
public class MyChefCommentAdapter  extends SectionedRecyclerViewAdapter<CommentHolder, ImageHolder,FootHolder> {


    public List<MyChefCommentBean.CookercommentBean> allTagList;
    ...
    public MyChefCommentAdapter(Context context) {
        mContext = context;
        mInflater = LayoutInflater.from(context);
        mBooleanMap = new SparseBooleanArray();
    }

    public void setData(List<MyChefCommentBean.CookercommentBean> allTagList) {
...
//        }
    ...
    protected void onBindSectionFooterViewHolder(FootHolder holder, final int section) {
        Glide.with(MyApplication.getContext()).load(Constants.BaseUrl+allTagList.get(section).getCookimg()).into(holder.footimage);
        holder.comment_name.setText(allTagList.get(section).getCookname());
        holder.comment_price.setText("¥ "+allTagList.get(section).getPrice());
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                mOnLeftItemClickListener.onMYChefCommentClick(section);
            }
        });
    }
    ...
    protected void onBindItemViewHolder(final ImageHolder holder, final int section, final int position) {
        ...
        if (mOnLeftItemClickListener != null) {
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    mOnLeftItemClickListener.onMyChefItemClick(section, position);
                }
            });
        }
    }
    public interface MyChefCommentItemClickListener{
        void onMyChefItemClick(int section,int position);
        void  onMYChefCommentClick(int position);
    }
    private MyChefCommentItemClickListener mOnLeftItemClickListener;

    public void setMyChefCommentItemClickListener(MyChefCommentItemClickListener onLeftItemClickListener) {
