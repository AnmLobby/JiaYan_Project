...
public class MainClassifyPresenter extends AbstractMvpPersenter<MainClassifyView> {
    ...
    public void clickRequestClassifyChef() {
        if (getmMvpView() != null) {
            getmMvpView().requestLoading();
        }
        //模拟网络延迟，可以显示出加载中
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                chefModel.getClassifyChef(new IBaseRequestCallBack<MainChefClassifyBean>() {
                    @Override
                    public void requestError(Throwable throwable) {
                        if (getmMvpView() != null) {
                            getmMvpView().resultFailure(Log.getStackTraceString(throwable));
                    ...
                    @Override
                    public void requestSuccess(MainChefClassifyBean cartBean) {
                        if (getmMvpView() != null) {
